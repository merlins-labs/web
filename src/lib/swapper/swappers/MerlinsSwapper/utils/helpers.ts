...
import type { merlins } from '@shapeshiftoss/chain-adapters'
import { toAddressNList } from '@shapeshiftoss/chain-adapters'
import type { CosmosSignTx, Merlins } from '@shapeshiftoss/hdwallet-core'
...
import { osmoService } from 'lib/swapper/swappers/MerlinsSwapper/utils/furyService'
import type {
  IbcTransferInput,
  MerlinsSupportedChainAdapter,
  PoolInfo,
  PoolRateInfo,
} from 'lib/swapper/swappers/MerlinsSwapper/utils/types'
...
const findPool = async (
  ...
  const poolsUrl = osmoUrl + '/lcd/merlins/gamm/v1beta1/pools?pagination.limit=1000'
...
type PerformIbcTransferInput = {
  ...
  adapter: MerlinsSupportedChainAdapter
  blockBaseUrl: string
  denom: string
...
export const buildSwapExactAmountInTx = async ({
  ...
  return {
    tx,
    addressNList: toAddressNList(bip44Params),
    chain_id: CHAIN_REFERENCE.MerlinsMainnet,
    account_number,
    sequence,
  }
}
