...
import { fromAccountId, merlinsAssetId } from '@shapeshiftoss/caip'
...
import { MERLINS_PRECISION } from 'state/slices/opportunitiesSlice/resolvers/merlins/utils'
...
export const Status: React.FC<StatusProps> = ({ accountId }) => {
  ...
  if (!feeAsset) throw new Error(`Fee asset not found for AssetId ${merlinsAssetId}`)
  const feeAssetMarketData = useAppSelector(state =>
    selectMarketDataById(state, merlinsAssetId ?? ''),
  )
  ...
  const lpAsset = useAppSelector(state => selectAssetById(state, merlinsOpportunity?.assetId ?? ''))
  ...
  if (!underlyingAsset0)
    throw new Error(`Asset not found for AssetId ${merlinsOpportunity?.underlyingAssetIds[0]}`)
  if (!underlyingAsset1)
    throw new Error(`Asset not found for AssetId ${merlinsOpportunity?.underlyingAssetIds[1]}`)
  ...
  useEffect(() => {
    if (confirmedTransaction && confirmedTransaction.status !== 'Pending' && contextDispatch) {
      contextDispatch({
        type: MerlinsWithdrawActionType.SET_WITHDRAW,
        payload: {
  ...
  if (!state || !merlinsOpportunity) return null
      ...
      case 'success':
        return {
          ...
          statusBody: translate('modals.withdraw.status.success', {
            opportunity: merlinsOpportunity?.name,
          }),
          statusBg: 'green.500',
        }
      case 'failed':
  ...
  return (
    <TxStatus
      ...
      pairIcons={merlinsOpportunity?.icons}
    >
